...
class _Log:
    ...
    def d(self, data):
        self.logger.info(
            '[' + self._str_complement('INFO', 8) + ' @ ' + str(timezone.now()) + '/' + BUILD_NAME + ' v' + str(
                BUILD_VERSION) + '] // '
            + self._str_complement(str(self.comp), 10) + ' //: ' + str(data))
        print('[' + self._str_complement('INFO', 8) + ' @ ' + str(timezone.now()) + '/' + BUILD_NAME + ' v' + str(
            BUILD_VERSION) + '] // '
              + self._str_complement(str(self.comp), 10) + ' //: ' + str(data))

    def warn(self, data):
        self.logger.warning(
            '[' + self._str_complement('WARNING', 8) + ' @ ' + str(timezone.now()) + '/' + BUILD_NAME + ' v' + str(
                BUILD_VERSION) + '] // '
            + self._str_complement(str(self.comp), 10) + ' //: ' + str(data))

    def shout(self, data):
        self.logger.critical(
            '[' + self._str_complement('CRITICAL', 8) + ' @ ' + str(timezone.now()) + '/' + BUILD_NAME + ' v' + str(
                BUILD_VERSION) + '] // '
            + self._str_complement(str(self.comp), 10) + ' //: ' + str(data))
...
def get_component_list():
    return (
    SiteComponentUrl().mk(name=u'НОВОСТИ', url='index'),
    SiteComponentUrl().mk(name=u'ПРОФИЛЬ', url='profile'),
    SiteComponentUrl().mk(name=u'МИССИИ', url='missions'),
    SiteComponentUrl().mk(name=u'ТРЕНИРОВКА', url='train'),
    SiteComponentUrl().mk(name=u'СКЛАД', url='stash'),
    SiteComponentUrl().mk(name=u'СЮЖЕТ', url='nfo.storyline'),
    SiteComponentUrl().mk(name=u'УФОПЕДИЯ', url='nfo.rnd'),
    SiteComponentUrl().mk(name=u'ЗАПИСАТЬСЯ В XCOM', url='nfo.recruit'),
    SiteComponentUrl().mk(name=u'VK', url='vk'),
    )
...
class site(SiteComponent):
    ...
    class profile(SiteComponent):
        ...
        class users(SiteComponent):
            ...
            def edit(self, request, user_id):
                if request.method == 'GET':
                    ...
                    return my_render_wrapper(request, 'app/profile/users/edit.html',
                                             {'form': UserEditForm(), 'target_user': target_user})
        ...
        class chars(SiteComponent):
            ...
            def view(self, request, char_id):
                ...
                context = {'user': request.user, 'char': char, 'items': request.user.account.items.all(),
                           'abilities': char.abilities.all(), }
    ...
    class stash(SiteComponent):
        ...
        class token(SiteComponent):
            ...
            def _transfer_it(self, source_user, item_token, qty, target_user):
                ...
                self.Log.d('[' + s + '] -> [' + target_user.username + '], item ' + str(item_token.item) + ' x' + str(
                    qty) + '.')
                return None
            ...
            def buy(self, request, token_id):
                ...
                if request.method == 'GET':
                    ...
                    if form.is_valid() and int(form.cleaned_data['count']) <= int(it.count) and int(your_money) >= int(
                            form.cleaned_data['count']) * int(it.price):
                        # money reduction
                        acc = request.user.account
                        acc.money -= form.cleaned_data['count'] * it.price
                        acc.save()
                        # item transfer
                        self._transfer_it(get_object_or_404(User, username='root'), it, form.cleaned_data['count'],
                                          request.user)

                        return HttpResponseRedirect(reverse('app:stash.view'))
                    else:
                        print(your_money, form.cleaned_data['count'] * it.price)
                        print(form.is_valid(), form.cleaned_data['count'] <= it.count,
                              your_money < form.cleaned_data['count'] * it.price)
    ...
    class train(SiteComponent):
        ...
        class neuro(SiteComponent):
            ...
            def auth(self, request, nrq_id):
                ...
                self.Log.d('Nrq id ' + str(nrq.pk) + ' from {} to {} by {} authorized '.format(nrq.teacher, nrq.pupil,
                                                                                               nrq.target_class))
    ...
    class nfo(SiteComponent):
        ...
        def vk(self, request):
            return HttpResponseRedirect('http://vk.com/xcom40000')
